...
exports.start = async(lol, name) => {
    text = `Hello ${name}! Ini adalah bot SimSimi yang bisa kamu gunakan ketika kamu gabut.`
    await lol.replyWithMarkdown(text, { disable_web_page_preview: true })
    text = `Untuk memulai silahkan ketik Hai atau Halo`
    await lol.replyWithMarkdown(text, { disable_web_page_preview: true })
    text = `Jika terjadi error pada bot ini, silahkan lapor [kesini](${config.ownerLink}) ya ☺️`
    await lol.replyWithMarkdown(text, { disable_web_page_preview: true })
    text = `Selamat Menggunakan :)️`
    await lol.replyWithMarkdown(text, { disable_web_page_preview: true })
}
